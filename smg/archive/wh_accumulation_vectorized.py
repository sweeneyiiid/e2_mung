...
coll_name = "historical_rekey_prev"

out_coll_name = "wh_accumulation_test"

db = client[db_name]

# Set index so sort will work
# https://stackoverflow.com/questions/26375017/mongo-error-when-using-aggregation-sort-exceeded-memory-limit
# commented out because you only have to do it once
db[coll_name].create_index("rawExtKey")
...
pipeline = [
    ...
    {
        '$sort':  { 'rawExtKey': 1}#, 'time_stamp': 1 }
    },
    {
        '$group':  {
            '_id': '$rawExtKey',
            'productUse': {'$first':'$productUse'},
            'connectionDate': {'$first':'$connectionDate'},
            'customer': {'$first':'$customer'},
            'microgrid': {'$first':'$microgrid'},
            'whTransactions': {'$first':'$whTransactions'},
            'extKey': {'$first':'$extKey'},
